...
export class ServicioDBService {
  ...
  modificarCantidadesporFactor(factor: number): Promise<any> {
    const query = 'UPDATE ingrediente SET cantidad = cantidad * ?';
    return this.database.executeSql(query, [factor]).then(() => {
      this.buscarIngredientes();
    });
  }
